...
export default function CourseSelectModal({
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        if (failCount > 0) {
            toast.warning(`Guardado parcial: ${successCount} OK, ${failCount} Error.`);
        } else {
      ...
      <div className="bg-card border border-white/10 rounded-xl shadow-2xl p-6 w-full max-w-6xl max-h-[90vh] flex flex-col">
          <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/10">
            ...
            <div className="flex items-center gap-4">
              ...
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary text-foreground">
              ...
              return sortedKeys.map((sem) => (
                <div key={sem} className="mb-8">
                  <h3 className="text-lg font-semibold text-foreground/80 mb-4 flex items-center gap-3">
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-secondary text-primary font-bold text-sm border border-white/5">{sem}</span>
                    Semestre {sem}
                  </h3>
                    ...
                    {coursesBySemester[sem].map((course) => {
                      ...
                      return (
                        <div key={course.codigo} className="relative group">
                            {!existsGroups && !isSel && (
                                <div className="absolute -top-2 -right-2 z-10 bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full shadow-sm font-bold pointer-events-none">Sin Grupos</div>
                            )}
                            <CourseSelectCard course={course} selected={isSel} disabled={isDisabled || isSaving} onToggle={() => toggleCourse(course)} onShow={(c) => setDetailCourse(c)} />
                        </div>
                      );
                    })}
                  </div>
          ...
          <div className="mt-6 pt-4 border-t border-white/10 flex justify-between items-center">
            <span className="text-sm text-muted-foreground">{selected.length === 0 ? "Selecciona una materia para comenzar" : `${selected.length} materias seleccionadas`}</span>
            <div className="flex gap-3">
              <button onClick={onClose} disabled={isSaving} className="px-6 py-2 rounded-lg hover:bg-white/5 text-sm font-medium transition-colors text-foreground">Cancelar</button>
              <button onClick={handleSave} disabled={selected.length === 0 || isSaving} className="px-8 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-bold shadow-lg shadow-primary/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95 flex items-center gap-2">
                {isSaving && <Loader2 className="w-4 h-4 animate-spin"/>} {isSaving ? "Guardando..." : "Confirmar Selección"}
              </button>
            </div>
          </div>
      </div>
